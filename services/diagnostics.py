...
def format_report(report: Dict[str, Any]) -> str:
    ...
    lines.append(f"base_url={cfg.get('base_url')}, api_key_present={cfg.get('api_key_present')}, agent_mode={cfg.get('agent_mode')}, agent_model={cfg.get('agent_model')}, vec_store_count={cfg.get('vector_store_ids_count')}")
    for c in report.get("checks", []):
        ...
        if isinstance(d, dict):
            if "model" in d:
                msg += f" [model={d.get('model')}]"
            if "status_code" in d:
                msg += f" [status={d.get('status_code')}]"
            if "output" in d:
                out = d.get("output") or ""
                if out:
                    msg += f" out={out[:80].replace('\\n',' ')}"
        if c.get("error"):
            msg += f" ERR={str(c['error'])[:200]}"
        lines.append(msg)
    return "\n".join(lines)
